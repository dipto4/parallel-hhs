...
__device__ void __forceinline__ _upsweep(int* _arr_slow, int* _arr_fast, int tid) {
    for(int stride = 1; stride<BLOCKSIZE; stride<<=1) {
        int idx = (tid + 1) * (stride << 1) - 1;
        if(idx < BLOCKSIZE) {
            _arr_slow[idx] += _arr_slow[idx-stride];
            _arr_fast[idx] += _arr_fast[idx-stride];
        }
        __syncthreads();
    }
}
...
template<int BLOCKSIZE>
__device__ void __forceinline__ _downsweep(int* _arr_slow, int*  _arr_fast, int tid) {
...
__global__ void scan_predicate(T* __restrict__ total, T* __restrict__ slow, T* __restrict__ fast, const T pivot, partition* part) {
    ...
    _upsweep<BLOCKSIZE>(_scan_pred_slow, _scan_pred_fast, tid);
    
    if(tid == 0) {
        part->N_slow = _scan_pred_slow[BLOCKSIZE-1] ;
        part->N_fast = _scan_pred_fast[BLOCKSIZE-1] ;
    ...
    // downsweep time now
    _downsweep<BLOCKSIZE>(_scan_pred_slow, _scan_pred_fast, tid);
    
    if(_pred_slow[tid]) slow[_scan_pred_slow[tid]] = g_total[_scan_pred_slow[tid]];
    if(_pred_fast[tid]) fast[_scan_pred_fast[tid]] = g_total[_scan_pred_fast[tid]];
